...
select ItemID from Item where Currently >= (select MAX(Currently) from (select * from Item where Ends >= "2001-12-20 00:00:01") s where ItemID IN (select ItemID from ItemBid));
...
select count(*) from ((select SellerID from Seller) INTERSECT (select BidderID from Bidder)) s;
